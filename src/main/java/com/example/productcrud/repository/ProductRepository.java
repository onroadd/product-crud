...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p FROM Product p WHERE p.category.user = :user AND (:keyword IS NULL OR LOWER(CAST(p.name AS string)) LIKE LOWER(CONCAT('%', :keyword, '%')) AND (:category IS NULL OR p.category = :category))")
